...
export default function MobileTabBar() {
  ...
  return (
    <>
      <nav
        className="md:hidden fixed bottom-0 inset-x-0 border-t border-[#1B4167]/30 bg-[#06101F]/90 backdrop-blur-xl flex justify-around items-center z-30 px-2 py-2"
      ...
      >
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col items-center justify-center flex-1 py-1 gap-1 transition-colors duration-200 ${
                isActive ? 'text-focus font-medium' : 'text-textMuted hover:text-textSecondary'
              }`
            }
          >
            {({ isActive }) => (
              <>
                ...
                {/* Active indicator dot with electric cyan glow */}
                {isActive && (
                  <span className="absolute bottom-[calc(max(env(safe-area-inset-bottom,0px),0.5rem)+2px)] w-1 h-1 rounded-full bg-focus shadow-[0_0_8px_#00C9C8]" />
                )}
              </>
            )}
          </NavLink>
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center flex-1 py-1 gap-1 transition-all duration-150 ${
            isOnMoreOnlyPage ? 'text-focus font-medium' : 'text-textMuted hover:text-textSecondary'
          }`}
